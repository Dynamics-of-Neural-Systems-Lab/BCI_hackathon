...
ATT_DIR = "/home/moritz/wiki/data/10/7a0ce5-2537-42da-907e-fb785c61a62f"  # TODO need to replace
...
def emg_fourier_embedding(
    emg_signal, n_points=128, sample_rate=200, min_freq=20, max_freq=150, skip_freqs=4
):
...
data_config = creating_dataset.DataConfig(
    **data_paths, window_size=window_size, samples_per_epoch=1000
)
...
preprocessed_train_data = [
    {
        "images": preprocess(train_dataset[i][1]),
...
preprocessed_test_data = [
    {
        "images": preprocess(test_dataset[i][1]),
...
unet(
    sample_image,
    encoder_hidden_states=preprocessed_train_data[0]["conditions"].unsqueeze(0),
    timestep=0,
).sample.shape
...
def evaluate(config, epoch, unet, scheduler, batch_size=16, guidance_scale=1.0):
    ...
    sample = torch.randn(
        (
            ...
            unet.config.sample_size[0],
            unet.config.sample_size[1],
        ),
        device=device,
    )
    ...
    print(
        "Evaluation MSE loss: ",
        F.mse_loss(sample, test_samples["images"].to(device)).item(),
    )
